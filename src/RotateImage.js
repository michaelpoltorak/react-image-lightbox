...
class ReactImageLightboxRotate extends Component {
  ...
  handleMovePrev() {
    if (
      ...
      (this.state.rotate > 0 || this.state.prevRotate > 0)
    ) {
      return () => {
  ...
  handleMoveNext() {
    if (
      (this.props.onMoveNextRequest && this.state.rotate > 0) ||
      this.state.nextRotate > 0
    ) {
      return () => {
  ...
  render() {
    ...
    let rotateLeftButtonHandler = () => {
      this.changeRotation(-90);
    };
    let rotateRightButtonHandler = () => {
      this.changeRotation(90);
    };
...
ReactImageLightboxRotate.defaultProps = Object.assign(
  {},
...
//module.exports = require('./override.js').default;
